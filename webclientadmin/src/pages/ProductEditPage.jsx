...
const ProductEditPage = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setProduct({
          ...productData,
          price: productData.price.toString() // number to string for input
        });
        
        // Customization seçeneklerini ayarla
        if (productData.customizationOptions) {
          const fields = Object.entries(productData.customizationOptions).map(([name, options]) => ({
            name,
            options: Array.isArray(options) ? options.join(',') : options
          }));
          setCustomizationFields(fields.length > 0 ? fields : [{ name: '', options: '' }]);
        } else {
          setCustomizationFields([{ name: '', options: '' }]);
        }
      } catch (err) {
        setError('Ürün yüklenirken bir hata oluştu: ' + err.toString());
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError('Ürün güncellenirken bir hata oluştu: ' + err.toString());
    } finally {
      setSaving(false);
    }
  };
